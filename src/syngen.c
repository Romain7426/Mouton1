...
static void queue(int type, int val, const char *name);
static void queue_cmp(int op);
static void genbranch(int dest, int inv);
...
char * labname(int id) {
  // static char name[100]; // Putain, il l'alloue sur la pile!!
  // sprintf(name, "%c%d", LPREFIX, id); // Putain, aucun check sur la taille!
  char * name;
  name = NULL;
  asprintf(&name, "%c%d", LPREFIX, id);
  return name;
}

char *gsym(const char *s) {
  // static char name[NAMELEN+2]; // Putain, il l'alloue sur la pile!!
  char * name;
  name = NULL;
  name = (char *) malloc((NAMELEN+2) * (sizeof (char)));
  name[0] = PREFIX;
  copyname(&name[1], s);
  return name;
}
...
void genname(const char *name) {
 genraw(gsym(name));
 genraw(":");
}

void genpublic(const char *name) {
 cgpublic(gsym(name));
}
...
void queue(int type, int val, const char *name) {
 commit();
 Q_type = type;
 Q_val = val;
