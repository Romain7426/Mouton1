...
// NB: His "synthesizing generator" is just an optimization of the stack-based generator.
// It is not a 'rupture'/'breaking' innovation.
// 
// The stack-based generator loads the values into registers, do the operations, then push back.
// The synthesizing generator skips the load of one of the operand, using the complex form of the i386 operations.
// In short, it is 
//   - stack-based generator: "load into eax", "mov into ecx", "load into eax", "xorl ecx, eax", "write eax" [5 operations] (4 if we load directly into ecx)
//   - synthesizing generator: "load into eax", "xorl <an addr>, %eax", "write eax" [3 operations]
//
// … Alright!
#include "defs.h"
#include "data.h"
...
void cglit(int v)	{ ngen("%s\t$%d,%%eax", "movl", v); } // load literal into EAX
void cgclear(void)	{ gen("xorl\t%eax,%eax"); }
void cgldgb(char *s)	{ sgen("%s\t%s,%%al", "movb", s); } // load global byte
void cgldgw(char *s)	{ sgen("%s\t%s,%%eax", "movl", s); } // load global word
void cgldlb(int n)	{ ngen("%s\t%d(%%ebp),%%al", "movb", n); } // load local byte
void cgldlw(int n)	{ ngen("%s\t%d(%%ebp),%%eax", "movl", n); } // load local word
void cgldsb(int n)	{ lgen("%s\t%c%d,%%al", "movb", n); }
void cgldsw(int n)	{ lgen("%s\t%c%d,%%eax", "movl", n); }
void cgldla(int n)	{ ngen("%s\t%d(%%ebp),%%eax", "leal", n); } // load local address
void cgldsa(int n)	{ lgen("%s\t$%c%d,%%eax", "movl", n); }
void cgldga(char *s)	{ sgen("%s\t$%s,%%eax", "movl", s); } // load global address
void cgindb(void)	{ gen("movl\t%eax,%edx");
			  cgclear();
			  gen("movb\t(%edx),%al"); } // load byte from address in EAX
void cgindw(void)	{ gen("movl\t(%eax),%eax"); } // load long from address in EAX
void cgargc(void)	{ gen("movl\t8(%ebp),%eax"); } 
void cgldlab(int id)	{ lgen("%s\t$%c%d,%%eax", "movl", id); }

void cgpush(void)	{ gen("pushl\t%eax"); }
void cgpushlit(int n)	{ ngen("%s\t$%d", "pushl", n); } // push literal
